...
oSelectList.onclick = function(e, a) {
	...
	var tagParent = tag.closest("dl").getElementsByTagName("dt").item(0).innerText;
	...
	if (tagName == 'INPUT') {
		...
		if (_tag_type_toUpper == 'CHECKBOX') { // 如果点击 的是 input checkbox
			...
			radioVals.push({
				key : $(tag).closest("dl").children().prev().html(),
				val : radioVal,
				type : 'zdy'
			})
			isCusPrice = true;
		...
		if (!oPrevInput) { // 如果上一个节点没有则认为点击的是 '不限'
			...
			if (trim(prev(parent).innerHTML) == tagParent) { // 这里是直接根据
																// text来比较的.建议加个自定义属性作标识符
	...
	if (radioVals.length > 0) {
		for (var i = 0, len = radioVals.length; i < len; i++) {
			infor += '<div class=\"selectedInfor selectedShow\"><span>'
					+ radioVals[i].key + '</span><label>' + radioVals[i].val
					+ '</label><em type="' + radioVals[i].type
					+ '"></em></div>';
		}
	}

	// if (radioVal) infor += '<div class=\"selectedInfor
	// selectedShow\"><span>活动价格</span><label>' + radioVal + '</label><em
	// p="2"></em></div>';
	...
	for (var i = 0, size = okSelect.length; i < size; i++) {
		...
		infor += '<div class=\"selectedInfor selectedShow\"><span>'
				+ vals[1].trim() + '</span><label>' + vals[0].trim()
				+ '</label><em></em></div>';
	}
...
function bindingEmClick() {
	...
	$('div.clearList em').on(
			...
			function() {
				var self = $(this);
				var val = trim(self.prev().html()) + '|'
						+ trim(self.prev().prev().html());
				var n = -1;
				var a = this.getAttribute('type') || 1;
				// self.die('click'); //die() 方法移除所有通过 live()
				// 方法向指定元素添加的一个或多个事件处理程序
...
function parents(elem, parents) { // 查找当前祖先辈元素需要的节点 如 parents(oDiv, 'dd') 查找
									// oDiv 的祖先元素为dd 的
	if (!elem || !parents)
		return;
