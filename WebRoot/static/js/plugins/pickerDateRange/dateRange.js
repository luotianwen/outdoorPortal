...
*/
    ...
    var defaults = {
        ...
        minValidDate : '315507600', //最小可用时间，控制日期选择器的可选力度（单位：秒）
    ...
    //让用户点击页面即可关闭弹窗
    $(document).bind('click', function (e,type) {
    	// 排除自动轮播事件和标签触发事件
    	if(type == "auto" || $(e.target).attr("show-type") == "span_in_time"){
    		return;
    	}
       __method.close();
    });
...
pickerDateRange.prototype.init = function(isCompare) {
	...
	if(this.mOpts.magicSelect && this.mOpts.theme == 'ta'){
		...
		for(var i = 0; i < this.mOpts.calendars; i ++) {
			...
			var td = null;
			if(this.mOpts.theme == 'ta'){
				td = this.fillDate(endDate.getFullYear(), month, i);
			}
			else{
				td = document.createElement('td');
				$(td).append(this.fillDate(endDate.getFullYear(), month, i));
			...
			month=month-2;//	endDate.setMonth(endDate.getMonth() + 1, 1);
		}
		 // 日历开始时间
		this.calendar_startDate = new Date(endDate.getFullYear(), endDate.getMonth(), 1);
	}
...
pickerDateRange.prototype.show = function(isCompare, __method) {
	...
	$("#" + this.calendarId).css('top', pos.top + (__method.mOpts.theme=='ta'?44:22) + 'px');
	//第一次显示的时候，一定要初始化输入框
