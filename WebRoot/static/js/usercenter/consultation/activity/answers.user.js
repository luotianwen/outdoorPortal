$(function(){
	...
	$.fn.consultation.prototype={
		...
		_load:function(page){
			var _this = this;
			//layer.msg("加载中...",{icon:16,shade:0.5,time:10*1000});
			$.post("activeConsultation/loadConsultation.do",{
		...
		_success:function(data){
			...
			for(var i=0,len=list.length;i<len;i++){
				...
				str +='<dl  class="user-questions">'
					+'<dt>'
					+'<div class="user-question">'+item.ac_comment+'<span class="date">'+item.create_time_str+'</span></div>'
					+'</dt>'
					+'<dd>';
					
					if(item.ac_is_reply == 0){
						...
						str +='<div class="answers">'
						+'<em class="m-1">'+item.replyUname+'</em>'
						+'回复'
						+'<em class="m-2">我</em>：'
						+item.ac_reply_comment
						...
						for(var j=0;j<item.acrs.length;j++){
							...
							str +='<div class="answers">'
							+'<em class="m-1">'+ac.acr_create_user_name+'</em>'
							+'回复'
							+'<em class="m-2">'+ac.acr_ac_user_name+'</em>：'
							+ac.acr_comment
					...
					if(item.ac_is_reply == 1 && item.ac_create_user_id == currentUserId){
						// 回复框
						str	+='<div class="input-block">'
							+'<input type="text" name="comment" placeholder="追问内容..." maxlength="70" />' 
							+'<a href="javascript:void(0);" class="answer-btn" id="reply_btn" reply-type="'+item.ac_is_reply+'" consultation-id="'+item.ac_id+'" reply-by-user="'+item.ac_reply_user_id+'" >追问</a>'
							+'</div>';
					}
