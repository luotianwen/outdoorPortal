/**
 * detail's plugins
 * include : lcSlider.js  jquery.fullcalendar.min.js  jquery.raty.min.js
 */
(function($) {
    //========================
    //图片缩放lcTpsf
    //========================
    $.fn.lcTpsf = function(options) {
        var options = $.extend({}, {
            maxWidth: 810,
            maxHeight: 452
        }, options);
    ...
    var oPic = G("imgs-slider");
    var oList = G("imgs-slider-listBox");
    var imgs = $(oPic).find("img");
    var maxWidth = $(oPic).width()
    imgs.lcTpsf({
        maxWidth: maxWidth
    })
    var lis = $(oPic).find("li")
    var oPrev = G("slider-prev");
    var oNext = G("slider-next");
    ...
    var index = 0;
    var listShowNum = Math.floor($("#imgs-slider-listBox").width() / 104)
    var num = listShowNum < len2 ? listShowNum : listShowNum * 2;
    ...
    $("#imgs-slider").parent().hover(function() {
...
(function($, undefined) {
    var defaults = {
        ...
        titleFormat: {
            month: 'yyyy 年 MMMM',
            week: "yyyy年MMM月d日{' 至 'yyyy年MMM月d日}",
    ...
    function Calendar(element, options, eventSources) {
        ...
        function renderView(inc) {
            if (elementVisible()) {
                ...
                header.updateTitle(currentView.price);
                var today = new Date();
    ...
    function MonthView(element, calendar) {
        ...
        function render(date, delta) {
            ...
            t.price = formatDate(start, opt('titleFormat'));
            t.start = start;
            t.end = end;
            t.visStart = visStart;
            t.visEnd = visEnd;
    ...
    function BasicWeekView(element, calendar) {
        ...
        function render(date, delta) {
            ...
            t.price = formatDates(visStart, addDays(cloneDate(visEnd), -1), opt('titleFormat'));
            t.start = start;
            t.end = end;
            t.visStart = visStart;
            t.visEnd = visEnd;
    ...
    function BasicDayView(element, calendar) {
        ...
        function render(date, delta) {
            ...
            t.price = formatDate(date, opt('titleFormat'));
    ...
    function BasicView(element, calendar, viewName) {
        ...
        function setHeight(height) {
            ...
            bodyFirstCells.each(function(i, _cell) {
                if (i < rowCnt) {
                    cell = $(_cell);
                    setCSSHeight(cell.find('> div'), (i == rowCnt - 1 ? rowHeightLast : rowHeight) - vsides(cell))
                }
            })
        }
    ...
    function AgendaEventRenderer() {
            ...
            for (i = 0; i < segCnt; i++) {
                ...
                if (eventElement = seg.element) {
                    ...
                    if (seg.contentTop !== undefined && height - seg.contentTop < 10) {
                        eventElement.find('div.fc-event-time').text(formatDate(event.start, opt('timeFormat')) + ' - ' + event.price);
        ...
        function slotSegHtml(event, seg) {
            ...
            html += " class='" + classes.join(' ') + "' style='position:absolute;z-index:8;top:" + seg.top + "px;left:" + seg.left + "px;" + skinCss + "'><div class='fc-event-inner fc-event-skin'" + skinCssAttr + "><div class='fc-event-head fc-event-skin'" + skinCssAttr + "><div class='fc-event-time'>" + htmlEscape(formatDates(event.start, event.end, opt('timeFormat'))) + "</div></div><div class='fc-event-content'><div class='fc-event-title'><a>" + htmlEscape(event.price) + "</a></div></div><div class='fc-event-bg'></div></div>";
            if (seg.isEnd && isEventResizable(event)) {
    ...
    function DayEventRenderer() {
        ...
        function daySegHTML(segs) {
            ...
            for (i = 0; i < segCnt; i++) {
                ...
                html += "<span class='fc-event-title'><a href='javascript:void(0);' onclick='showtuan(" + event.id +  ")' >" +htmlEscape(event.price) + "</a></span></div>";
                //html += "<span class='fc-event-title'><a target='_blank' href='/order/open?package=" + package_info.package + "&depart_date="+ formatDate(event.start, "yyyy-MM-dd") +"'>" + htmlEscape(event.price) + "</a></span></div>";
                if (seg.isEnd && isEventResizable(event)) {
...
(function($) {
    $.fn.kxbdSuperMarquee = function(options) {
        ...
        return this.each(function() {
            ...
            navHtml += '</ul>';
            if ((scrollSize - 40) < (_type ? scrollW : scrollH)) {
                $("#goL,#goR").remove();
                return;
            }
