	...
	$.fn.journey.prototype={
		_init:function($this){
			...
			$addAddress.on("click",function(){
				...
				// 添加新的元素
				var str = ' <label class="input-box">- <input name="didian['+$addressInputs.length+'].la_address_name" type="text" class="wid02"  placeholder="终点" maxlength="20" /><i class="input-icon-del"></i></label>';
				$(this).before(str);
	...
	$("a#add_new_line").on("click",function(){
		...
		lines.each(function(l_index){
			...
			if(l_index == lines.length-1){
				var time = new Date().getTime(),index=$("div.journey").length,journeyType=$("input[name=journey_type]:hidden:eq(0)").val();
